...
sample_BNP_biv_DPM_t sample_BNP_biv_DPM(List Y_bis, arma::mat Z, arma::mat B_Z, arma::cube theta, arma::mat X_Z, arma::mat X_Y, arma::vec U_Z, arma::mat s, arma::vec nj, double K_N, bool isDP, double kappa, double sigma, double u, arma::mat b_Z_star, arma::cube theta0_star, arma::vec m_b_Z, arma::mat Sigma_b_Z, arma::mat Omega_b_Z, arma::mat m_theta0, arma::cube Sigma_theta0, double sig2_Z, arma::vec sig2_Y, arma::vec subscales_indices_main){
  ...
  for(int i = 0; i < N; i++){ 
    ...
    for(int i_aux = 0; i_aux < N_aux; i_aux++){
      ...
      for(int t = 0; t < nT_Z; t++){
        f_k(i_aux) += arma::log_normpdf(Z_i(t), arma::accu(B_Z.row(t) % b_Z_aux.row(i_aux)) + arma::accu(X_Z_i % U_Z.t()), sqrt(sig2_Z));
      }
      
      //theta part (questionnaire)
      for(int is = 0; is < n_subscales_main; is++){
        for(int t = 0; t < nT_Y; t++){
          f_k(i_aux) += arma::log_normpdf(theta(i,is,t), theta0_aux(i_aux,is,t), sqrt(sig2_Y(is)));
        }
      }
    }
    ...
    for(int j_aux = 0; j_aux < K_N; j_aux++){
      ...
      //SPlines part (longitudinal)
      for(int t = 0; t < nT_Z; t++){
        f_k(N_aux + j_aux) += arma::log_normpdf(Z_i(t), arma::accu(B_Z.row(t) % b_Z_star.row(j_aux)) + arma::accu(X_Z_i % U_Z.t()), sqrt(sig2_Z));
      }
      
      //theta part (questionnaire)
      for(int is = 0; is < n_subscales_main; is++){
        for(int t = 0; t < nT_Y; t++){
          f_k(N_aux + j_aux) += arma::log_normpdf(theta(i,is,t), theta0_star(j_aux,is,t), sqrt(sig2_Y(is)));
        }
      }
    }
  ...
  for(int k = 0; k < K_N; k++){
    ...
    for(int is = 0; is < n_subscales_main; is++){
      ...
      for(int i = 0; i < N; i++){
        ...
        if(s_i == k){
          for(int t = 0; t < nT_Y; t++){
            m_theta0_post(t) += theta(i,is,t) / sig2_Y(is);
          }
        }
      }
  ...
  return sample_BNP_biv_DPM_t(s, nj, K_N, b_Z_star, theta0_star);
}
