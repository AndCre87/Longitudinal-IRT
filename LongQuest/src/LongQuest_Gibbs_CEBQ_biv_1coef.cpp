...
List LongQuest_Gibbs_CEBQ_biv_1coef(List data_list, List MCMC_list, List Param_list){
  ...
  arma::vec xi_Y(n_subscales_main, arma::fill::ones), s_xi_Y(n_subscales_main, arma::fill::ones), xi_Y_accept(n_subscales_main, arma::fill::zeros);
  arma::vec eta_Z(q_Z, arma::fill::ones);
  arma::mat eta_Y(q_Y, n_subscales_main, arma::fill::ones);
  ...
  arma::mat m_U_Y(q_Y, n_subscales_main, arma::fill::zeros), U_Y(q_Y, n_subscales_main, arma::fill::zeros), sum_U_Y(q_Y, n_subscales_main, arma::fill::zeros);
  if(U_Y_zeros){
    U_Y.zeros();
  }
  arma::cube Omega_U_Y(q_Y, q_Y, n_subscales_main, arma::fill::zeros), S_U_Y(q_Y, q_Y, n_subscales_main, arma::fill::zeros), prod_U_Y(q_Y, q_Y, n_subscales_main, arma::fill::zeros);
  for(int is = 0; is < n_subscales_main; is++){
    Omega_U_Y.slice(is).diag() = xi_Y(is) * eta_Y.col(is);
    S_U_Y.slice(is).diag().fill(0.01);
  }
  
  arma::cube eye_mat_U_Y(q_Y, q_Y, n_subscales_main, arma::fill::zeros);
  for(int is = 0; is < n_subscales_main; is++){
    eye_mat_U_Y.slice(is).eye();
  }
  
  Omega_U_Z.diag() = xi_Z * eta_Z;
  
  //Adaptive on regression coefficient
  arma::vec s_d_U_Y(n_subscales_main, arma::fill::zeros), U_Y_accept(n_subscales_main, arma::fill::zeros);
  ...
  arma::mat alpha_Y_out(n_save, JY, arma::fill::zeros), mu_alpha_Y_out(n_save, n_subscales, arma::fill::zeros), s_out(n_save, N, arma::fill::zeros), U_Z_out(n_save, q_Z, arma::fill::zeros), eta_Z_out(n_save, q_Z, arma::fill::zeros), sig2_Y_out(n_save, n_subscales_main, arma::fill::zeros), xi_Y_out(n_subscales_main, n_save, arma::fill::zeros);
  arma::cube Z_out(N, nT_Z, n_save, arma::fill::zeros), beta_Y_out(JY, m_Y, n_save, arma::fill::zeros), U_Y_out(q_Y, n_subscales_main, n_save), eta_Y_out(q_Y, n_subscales_main, n_save);
  ...
  for(int g = 0; g < n_tot; g ++){
      ...
      for(int j = 0; j < JY; j ++){
        ...
        for(int i = 0; i < N; i ++){
          ...
          if(!arma::is_finite(Y_t(i,j))){
            ...
            prob_h.zeros();
            for(int h = 0; h < m_Y; h++){
              prob_h(h) = alpha_Y_j * (h * theta(i,is,t) - beta_cumsum(h)) + h * arma::accu(X_Y_i % U_Y_is);
            }
        ...
        for(int t = 0; t < nT_Y; t++){
          ...
          for(int j = 0; j < JY; j++){
            ...
            if(subscales_indices_main(j) == is){
              ...
              prob_itj.zeros();
              for(int h = 0; h < m_Y; h++){
                prob_itj(h) = alpha_Y_j * (h * theta_new - beta_cumsum(h)) + h * arma::accu(X_Y_i % U_Y_is);
              }
              ...
              prob_itj.zeros();
              for(int h = 0; h < m_Y; h++){
                prob_itj(h) = alpha_Y_j * (h * theta(i,is,t) - beta_cumsum(h)) + h * arma::accu(X_Y_i % U_Y_is);
              }
              prob_itj = exp(prob_itj - max(prob_itj));
              prob_itj = prob_itj/sum(prob_itj);
    ...
    for(int j = 0; j < JY; j++){
      ...
      for(int t = 0; t < nT_Y; t ++){
        ...
        for(int i = 0; i < N; i ++){
          ...
          prob_itj.zeros();
          for(int h = 0; h < m_Y; h++){
            prob_itj(h) = alpha_Y_j * (h * theta(i,is,t) - beta_new_cumsum(h)) + h * arma::accu(X_Y_i % U_Y_is);
          }
          prob_itj = exp(prob_itj - max(prob_itj));
          prob_itj = prob_itj/sum(prob_itj);
      ...
      for(int j = 0; j < JY; j++){
        ...
        for(int t = 0; t < nT_Y; t ++){
          ...
          for(int i = 0; i < N; i ++){
            ...
            prob_itj.zeros();
            for(int h = 0; h < m_Y; h++){
              prob_itj(h) = alpha_Y_new * (h * theta(i,is,t) - beta_cumsum(h)) + h * arma::accu(X_Y_i % U_Y_is);
            }
            prob_itj = exp(prob_itj - max(prob_itj));
            prob_itj = prob_itj/sum(prob_itj);
    ...
    if(use_horseshoe){
      ...
      for(int is = 0; is < n_subscales_main; is++){
        for(int l = 0; l < q_Y; l++){
          mj = 0.0;
          mj += 0.5 * pow(U_Y(l,is) - m_U_Y(l,is), 2.0) * xi_Y(is);
          Uj = arma::randu() / (1 + eta_Y(l,is));
          Tj = (1/Uj - 1);
          uj = arma::randu();
          
          eta_Y(l,is) = -1/mj * log(1 - uj*(1 - exp(-mj * Tj)));
        }
      }
      ...
      //     double mj = 0.0, Uj = 0.0, Tj = 0.0, uj = 0.0;
      //     for(int l = 0; l < q_Y; l++){
      //       mj += 0.5 * pow(U_Y(l,is) - m_U_Y(l,is), 2.0) * eta_Y(l,is);
      //     }
      //     Uj = arma::randu() / (1 + xi_Y(is));
      //     Tj = (1/Uj - 1);
      //     uj = arma::randu();
      //     
      //     if(q_Z == 1){
      //       xi_Y(is) = - 1/mj * log(1 - uj*(1 - exp(-mj * Tj)));
      //     }else{
      //       xi_Y(is) = boost::math::gamma_p_inv((q_Y + 1.0)/2.0, uj * boost::math::gamma_p((q_Y + 1.0)/2.0, mj * Tj)  ) / mj;
      //     }
      //   }
      ...
      for(int is = 0; is < n_subscales_main; is++){
        arma::vec U_Y_is = U_Y.col(is), m_U_Y_is = m_U_Y.col(is);
        // Propose a new value
        double xi_Y_new = xi_Y(is) * exp(arma::randn() * sqrt(s_xi_Y(is)));
        
        // Compute MH ratio:
        
        //Proposal and prior
        double log_ratio_xi_Y = (q_Y * m_Y + 1.0)/2.0 * (log(xi_Y_new) - log(xi_Y(is))) - (log(1 + xi_Y_new) - log(1 + xi_Y(is)));
        
        //Regression coefficients part
        //Start from category 1 beause category 0 is fixed
        for(int h = 1; h < m_Y; h++){
          log_ratio_xi_Y += - 0.5 * arma::accu(eta_Y.col(is) % (U_Y_is - m_U_Y_is) % (U_Y_is - m_U_Y_is)) * (xi_Y_new - xi_Y(is));
        }
        ...
        xi_Y_accept(is) += accept_xi_Y;
        if( arma::randu() < accept_xi_Y ){
          xi_Y(is) = xi_Y_new;
        }
        
        s_xi_Y(is) += pow(g+1,-ADAPT(1))*(accept_xi_Y - ADAPT(2));
        if(s_xi_Y(is) > exp(50)){
          s_xi_Y(is) = exp(50);
        }else{
          if(s_xi_Y(is) < exp(-50)){
    ...
    if(!U_Y_zeros){
      ...
      for(int is = 0; is < n_subscales_main; is++){
        ...
        arma::vec U_Y_is = U_Y.col(is), m_U_Y_is = m_U_Y.col(is);
        arma::mat S_U_Y_is = S_U_Y.slice(is), Omega_U_Y_is = Omega_U_Y.slice(is);
        
        //Propose new vector of regression coefficients with random walk
        arma::vec U_Y_new = arma::mvnrnd(U_Y_is, S_U_Y_is);
        
        //Computing MH ratio:
        double log_ratio_U_Y = 0.0;
        //Prior and proposal (symmetric)
        log_ratio_U_Y += - .5 * (arma::as_scalar((U_Y_new.t() - m_U_Y_is.t()) * Omega_U_Y_is * (U_Y_new - m_U_Y_is) - arma::as_scalar((U_Y_is.t() - m_U_Y_is.t()) * Omega_U_Y_is * (U_Y_is - m_U_Y_is))));
        ...
        for(int t = 0; t < nT_Y; t++){
          ...
          U_Y_accept(is) += accept_U_Y;
          
          if( arma::randu() < accept_U_Y ){
            U_Y.col(is) = U_Y_new;
          }
          
          // sum_U_Y.col(is) += U_Y.col(is);
          // 
          // prod_U_Y.slice(is) += U_Y.col(is) * U_Y.col(is).t();
          // 
          // s_d_U_Y(is) += pow(g+1,-ADAPT(1))*(accept_U_Y - ADAPT(2));
          // if(s_d_U_Y(is) > exp(50)){
          //   s_d_U_Y(is) = exp(50);
          // }else{
          //   if(s_d_U_Y(is) < exp(-50)){
          //     s_d_U_Y(is) = exp(-50);
          ...
          // }
          // if(g > (ADAPT(0) - 1)){
          //   S_U_Y.slice(is) = s_d_U_Y(is)/g * (prod_U_Y.slice(is) - sum_U_Y.col(is) * sum_U_Y.col(is).t()/(g+1.0)) + s_d_U_Y(is) * pow(0.1,2.0) / q_Y * eye_mat_U_Y.slice(is);
          // }
        }
      }
    }
